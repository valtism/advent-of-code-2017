...
function run(input) {
    const programs = input.map(programs => programs.match(/\w+/g));
    const root = getRootProgram(programs);
    const nodeMap = createNodeMap(programs);
    const wrongNodes = findWrongNode(nodeMap);
    return wrongNodes;
}

function getRootProgram(programsInput) {
    const programs = [];
    const children = [];
    programsInput.forEach(programsArray => {
        const program = programsArray[0];
        const childArray = programsArray.slice(2);
        programs.push(program);
        children.push(...childArray);
    });
    return programs.find(program => !children.includes(program));
}

function createNodeMap(programsInput) {
    // Use a reduce instead of a map so properties can be accessed by name
    const nodeMap = programsInput.reduce(addNode, []);
    // Now all nodes are initialised, we can link children to them
    linkChildren(nodeMap);
    return nodeMap;
}

function addNode(nodeMap, programs) {
    const name = programs[0];
    const weight = Number(programs[1]);
    const children = programs.slice(2);
    const sumWeights = function() {
        return this.children.reduce(
            (weight, child) => weight + child.sumWeights(),
            this.weight
        );
    };
    nodeMap[name] = {
...
function linkChildren(nodeMap) {
    for (const name in nodeMap) {
        const node = nodeMap[name];
        node.children = node.children.map(child => nodeMap[child]);
    }
}

function findWrongNode(nodeMap) {
    const unbalancedNodes = [];
    for (const name in nodeMap) {
        const node = nodeMap[name];
        const childWeights = node.children.map(child => child.sumWeights());
        if (!isAllValuesEqual(childWeights)) {
            unbalancedNodes.push(node);
        }
    }
    const parentOfWrongNode = getLowestWeightNode(unbalancedNodes);
    return parentOfWrongNode.children.map(node => ({
        name: node.name,
        totalWeight: node.sumWeights()
    }));
}

function isAllValuesEqual(array) {
    if (!array) {
        return true;
    }
...
function getLowestWeightNode(nodes) {
    return nodes.reduce(
        (lowestWeightNode, node) =>
            lowestWeightNode.sumWeights() < node.sumWeights()
                ? lowestWeightNode
                : node
    );
}
