...
function run(input) {
    const registers = getRegisters(input);
    const instructions = getInstructions(input);
    const logs = [];

    instructions.forEach(instruction => {
        if (fulfils(instruction.condition, registers)) {
            registers[instruction.registerName] += instruction.change;
            logs.push(registers[instruction.registerName]);
        }
    });

    let highest = 0;
    for (const name in registers) {
        if (registers[name] > highest) {
            highest = registers[name];
        }
    }

    const highestHeld = logs.reduce(
...
function getRegisters(instructionArray) {
    const registers = instructionArray.reduce((registers, instruction) => {
...
function getInstructions(instructionArray) {
    const instructions = instructionArray.map(instruction => {
        const arr = instruction.split(" ");
        return {
            registerName: arr[0],
            change: arr[1] === "inc" ? Number(arr[2]) : -Number(arr[2]),
            condition: arr.slice(4)
        };
    });
...
function fulfils(condition, registers) {
